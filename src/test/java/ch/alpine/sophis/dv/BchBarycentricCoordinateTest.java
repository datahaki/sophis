...
class BchBarycentricCoordinateTest {
  ...
  void testSe2() {
    ...
    for (int n = 4; n < 7; ++n) {
      ...
      // ---
      Tensor seqG = Tensor.of(sequence.stream().map(Se2CoveringGroup.INSTANCE.lieExponential()::exp));
      BarycentricCoordinate bc = UsanceCoordinate.of(Se2CoveringGroup.INSTANCE, variogram);
      Tensor weights2 = bc.weights(seqG, Se2CoveringGroup.INSTANCE.lieExponential().exp(x));
