...
class CurveSubdivisionTest {
  ...
  void testSymmetric() {
    ...
    for (int length = 0; length < 10; ++length) {
      ...
      _checkSym(new BSpline1CurveSubdivision(RGroup.INSTANCE), tensor);
      _checkSym(new BSpline2CurveSubdivision(RGroup.INSTANCE), tensor);
      _checkSym(new BSpline3CurveSubdivision(RGroup.INSTANCE), tensor);
      _checkSym(BSpline4CurveSubdivision.split2lo(RGroup.INSTANCE), tensor);
      _checkSym(new FourPointCurveSubdivision(RGroup.INSTANCE), tensor);
      _checkSym(DodgsonSabinCurveSubdivision.INSTANCE, tensor);
      _checkSym(HormannSabinCurveSubdivision.split3(RGroup.INSTANCE), tensor);
    }
  }
