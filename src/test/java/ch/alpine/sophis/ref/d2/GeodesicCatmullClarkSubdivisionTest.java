...
class GeodesicCatmullClarkSubdivisionTest {
  ...
  void testQuad() {
    GeodesicCatmullClarkSubdivision catmullClarkSubdivision = //
        new GeodesicCatmullClarkSubdivision(RGroup.INSTANCE);
  ...
  void testRefineX() {
    ...
    assertEquals(refine, Tensors.vector(_ -> Subdivide.of(0, 1, 4), 9));
  ...
  void testRefineY() {
    ...
    Tensor refine = Nest.of(catmullClarkSubdivision::refine, grid, 2);
    assertEquals(Transpose.of(refine), Tensors.vector(_ -> Subdivide.of(0, 2, 8), 5));
    ExactTensorQ.require(refine);
  }
}
