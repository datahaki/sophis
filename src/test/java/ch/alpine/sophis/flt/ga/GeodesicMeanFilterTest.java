...
class GeodesicMeanFilterTest {
  ...
  void testSimple() {
    for (int radius = 0; radius < 4; ++radius) {
      TensorUnaryOperator tensorUnaryOperator = GeodesicMeanFilter.of(RGroup.INSTANCE, radius);
  ...
  @Test
  void testRadiusOne() {
    TensorUnaryOperator tensorUnaryOperator = GeodesicMeanFilter.of(RGroup.INSTANCE, 1);
    ...
    assertEquals(result.maps(Unitize.FUNCTION), expect);
  }
