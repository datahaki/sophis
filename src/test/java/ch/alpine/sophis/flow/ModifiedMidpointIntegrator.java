...
record ModifiedMidpointIntegrator(int n) implements TimeIntegrator, Serializable {
  ...
  public Tensor step(StateSpaceModel stateSpaceModel, Tensor x, Tensor u, Scalar h) {
    ...
    Tensor xm = x.add(stateSpaceModel.f(x, u).multiply(hn));
    for (int m = 1; m < n; ++m) {
    ...
    return x.add(stateSpaceModel.f(xm, u).multiply(hn));
  }
}
